...
import { Star, Heart, ShoppingBag, Truck, ShieldCheck, ChevronDown, ChevronUp, X, Ruler } from "lucide-react";
        ...
        <div className="w-full px-4 sm:px-6 lg:px-8 xl:px-12 grid grid-cols-1 md:grid-cols-12 gap-y-10 md:gap-x-8 lg:gap-x-12">
            ...
            <div className="md:col-span-5 relative px-0 md:px-4 lg:px-8 xl:pr-16 pt-8 md:pt-12">
                <div className="md:sticky md:top-24 space-y-8 pb-12">
                    ...
                    {/* MEGA DEAL CARD */}
                    {coupons.length > 0 && (
                        <div className="relative overflow-hidden rounded-xl border border-white/20 shadow-lg px-4 py-3"
                            style={{
                                background: `linear-gradient(to right, ${policies.mega_deal_bg_from || '#4f46e5'}, ${policies.mega_deal_bg_to || '#7c3aed'})`
                            }}>
                            <div className="absolute -top-4 -right-4 w-16 h-16 rounded-full bg-white/10 blur-xl pointer-events-none" />

                            {/* Badge */}
                            {policies.mega_deal_badge && (
                                <div className="absolute top-0 right-0 px-2 py-0.5 rounded-bl-lg bg-black/10 backdrop-blur-md border-l border-b border-white/10">
                                    <span style={{ color: policies.mega_deal_text_color || '#ffffff' }} className="text-[8px] font-black uppercase tracking-tighter opacity-80">
                                        {policies.mega_deal_badge}
                                    </span>
                                </div>
                            )}
                            ...
                            {coupons.map((coupon: any) => {
                                ...
                                if (coupon.type === 'percentage' && coupon.discount_amount) {
                                    const savings = (sellingPrice * parseFloat(coupon.discount_amount)) / 100;
                                    discountedPrice = sellingPrice - savings;
                                    savingsLabel = `${coupon.discount_amount}% off (${formatPrice(savings)} saved)`;
                                } else if (coupon.type === 'fixed' && coupon.discount_amount) {
                                    const savings = Math.min(parseFloat(coupon.discount_amount), sellingPrice);
                                    discountedPrice = sellingPrice - savings;
                                    savingsLabel = `${formatPrice(savings)} off`;
                                ...
                                return (
                                    <div key={coupon.id} className="flex items-center justify-between gap-3">
                                        ...
                                        <div className="flex items-center gap-2 min-w-0">
                                            <span className="text-base">{policies.mega_deal_icon || '⚡'}</span>
                                            <div className="min-w-0">
                                                <div className="flex items-baseline gap-1.5 flex-wrap">
                                                    <span style={{ color: policies.mega_deal_text_color || '#ffffff', opacity: 0.7 }}
                                                        className="text-[10px] font-black uppercase tracking-[0.15em] shrink-0">
                                                        {policies.mega_deal_label || 'Mega Deal'}
                                                    </span>
                                                    {discountedPrice !== null && (
                                                        <span style={{ color: policies.mega_deal_text_color || '#ffffff' }}
                                                            className="text-sm font-black whitespace-nowrap">
                                                            Get at {formatPrice(discountedPrice)}
                                                        </span>
                                                    )}
                                                    {coupon.type === 'free_shipping' && <span style={{ color: policies.mega_deal_text_color || '#ffffff' }} className="text-sm font-black whitespace-nowrap">🚚 Free Delivery</span>}
                                                    {coupon.type === 'bogo' && <span style={{ color: policies.mega_deal_text_color || '#ffffff' }} className="text-sm font-black whitespace-nowrap">🎁 {savingsLabel}</span>}
                                                </div>
                                                <p style={{ color: policies.mega_deal_subtext_color || '#c7d2fe' }}
                                                    className="text-[10px] font-semibold truncate mt-[-1px]">
                                                    {savingsLabel && discountedPrice !== null ? savingsLabel : ''}
                                                    {coupon.is_default_magic && ' · ✨ Pre-Applied'}
                                                </p>
                                            </div>
                                         </div>
                                         {/* Right: code chip */}
                                         <CouponChip code={coupon.code} parentTextColor={policies.mega_deal_text_color || '#ffffff'} parentSubtextColor={policies.mega_deal_subtext_color || '#c7d2fe'} />
                                     </div>
                                 );
                             })}
                         </div>
